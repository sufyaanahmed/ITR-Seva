...
function DraftErasure({ persistence, onErase }) {
  ...
  return (
    <section className="mt-8 border border-red-200 p-6 rounded bg-red-50" aria-labelledby="resume-erase-heading">
      ...
      <p className="text-sm text-red-900 mb-2">This clears your saved form answers and document selections from this browser session.</p>
      ...
      {!confirming ? (
        ...
        <div role="alertdialog" aria-labelledby="resume-confirm-heading" aria-describedby="resume-confirm-description" className="border border-red-300 bg-white p-4 rounded">
          ...
          <p id="resume-confirm-description" className="text-sm text-red-900 mb-4">This clears the visible draft and stored session state for this application. It cannot be undone.</p>
      ...
      {hasLocalPreparation ? (
        <div className="border-2 border-primary p-6 rounded-xl bg-primary-light/20 shadow-sm mb-10">
          ...
          <p className="text-sm text-text-secondary mb-4">{isComplete
            ? 'Your application dossier has been sealed.'
            : 'Your entered form fields and document selections were saved in your active session.'}</p>
          {reference && (
            <div className="bg-white/70 border border-primary/20 rounded p-3 mb-6">
              ...
              <strong className="font-mono text-sm break-all text-[#1E2A4F]">{reference}</strong>
            </div>
          )}
          <button type="button" onClick={() => navigate('/apply')} className="btn-primary w-full sm:w-auto cursor-pointer">{isComplete ? 'View Sealed Dossier' : 'Resume Application Draft'}</button>
        </div>
      ) : (
        <div className="border border-border p-8 rounded-xl bg-white shadow-sm mb-10 text-center">
          ...
          <p className="text-sm text-text-secondary mb-6">Start with the Visa Finder to determine your appropriate visa category and initialize an application.</p>
